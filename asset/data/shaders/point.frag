// ============================================================
// point.frag — 点渲染片段着色器
// 渲染管线中的作用：将点渲染为圆形，支持虚实融合深度检测。
// 使用 gl_PointCoord 将方形点图元裁剪为圆形（圆形外的片段 discard）。
// ============================================================
...
#define VIEW_DESCRIPTOR_SET 1        // 视图相关描述符集
#define MATERIAL_DESCRIPTOR_SET 2    // 材质相关描述符集
...
// 虚实融合纹理
layout(set = MATERIAL_DESCRIPTOR_SET, binding = 7) uniform sampler2D cameraImage;  // 相机画面
...
void main()
{
    // 圆形裁剪：gl_PointCoord 范围为 [0,1]，中心为 (0.5, 0.5)
    // 距离中心超过 0.5（半径）的片段被丢弃，将方形点渲染为圆形
    ...
    if(constantBuffer.shader_type == 0){
        ...
        // 虚实融合模式：比较虚拟点与真实场景的深度
        float cadDepth = -eyePos.z / constantBuffer.z_far;           // 虚拟点归一化深度
        float cameraDepth = texture(depthImage, screen_uv).r;        // 真实场景深度
        if(cadDepth > cameraDepth){
            // 虚拟点在真实物体后面 → 被遮挡 → 显示相机画面

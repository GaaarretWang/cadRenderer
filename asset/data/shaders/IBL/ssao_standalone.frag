...
// Define SSAO_RESOLVED before including for resolved (non-MSAA) variant
...
void main()
{
    ...
    vec3 normal = texelFetch(normalSampler, centerCoord, 0).rgb;
    ...
    vec3 randDir = texture(samplerNoise, uv).rgb * 2.0 - 1.0;
    vec3 tangent = normalize(randDir - normal * dot(randDir, normal));
    vec3 bitangent = cross(normal, tangent);
    mat3 TBN = mat3(tangent, bitangent, normal);

    float occlusion = 0.0;
    for (uint i = 0; i < uint(globalBuffer.ssao_kernel_size); ++i)
    {
        ...
        vec4 samplePosProj = pc.projection * pc.view * vec4(samplePos, 1.0);
        ...
        ivec2 sampleCoord = clamp(ivec2(sampleUV * vec2(inputSize)), ivec2(0), inputSize - 1);
        vec3 sceneWorldPos = texelFetch(worldPosSampler, sampleCoord, 0).rgb;
    ...
    float factor = 1.0 - (occlusion / float(globalBuffer.ssao_kernel_size));
    outColor = vec4(factor, factor, factor, 0.0);
}

...
ref_ptr<Node> Builder::createLine(const GeometryInfo& info, const StateInfo& stateInfo, std::vector<vec3> positionsInit, std::vector<uint32_t>& indicesInit)
{
    uint32_t instanceCount = 1;
    auto colors = instanceColors(info, instanceCount);

    size_t Nodenumber = positionsInit.size();
    size_t Indicesnumber = indicesInit.size();
    ...
    vsg::ref_ptr<vsg::vec2Array> texcoords = vsg::vec2Array::create(Nodenumber);
    vsg::ref_ptr<vsg::uintArray> indices = vsg::uintArray::create(Indicesnumber);

    for (int i = 0; i < Nodenumber; i++)
    {
        vertices->at(i) = vsg::vec3(positionsInit[i]);
        normals->at(i) = vec3(0, 1, 0);
        texcoords->at(i) = vec2(0, 0);
    }
    for (int i = 0; i < Indicesnumber; i++)
    {
        indices->at(i) = indicesInit[i];
    }
...
DataList Builder::createSphereWithArray(const GeometryInfo& info, const StateInfo& stateInfo)
{
    ...
    // auto color = vec4Value::create(info.color);

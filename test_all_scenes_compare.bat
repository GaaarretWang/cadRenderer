...
for /f "tokens=2 delims=:" %%A in ('findstr /R /C:"\"id\": [0-9][0-9]*" "%SCENES_JSON%"') do (
    for /f "tokens=1 delims=, " %%I in ("%%A") do (
        ...
        echo ----------------------------------------
        echo Running command: (cd /d "%BUILD_DIR%" ^&^& Rendering.exe --scene !SCENE_ID! --frames 2 --compare-ref!EXTRA_RENDER_ARGS!)

        pushd "%BUILD_DIR%" >nul
        "%EXE_PATH%" --scene !SCENE_ID! --frames 2 --compare-ref!EXTRA_RENDER_ARGS! > "!LOG_FILE!" 2>&1
        ...
        findstr /B /C:"PASS:" "!LOG_FILE!" >nul
        if not errorlevel 1 (
            echo OK Scene !SCENE_ID! ^(!SCENE_NAME!^) comparison passed
        ) else (
            findstr /B /C:"FAIL:" "!LOG_FILE!" >nul
            if not errorlevel 1 (
                echo X Scene !SCENE_ID! ^(!SCENE_NAME!^) comparison failed
            ) else (
                echo X Scene !SCENE_ID! ^(!SCENE_NAME!^) did not report a comparison result
            )
            set "SCENE_PASS=0"
        )
...
:show_help
echo Usage: test_all_scenes_compare.bat [extra Rendering args]
echo.
echo Compares all scene outputs against saved reference images.
exit /b 0
